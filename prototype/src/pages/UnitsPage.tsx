...
import { unitRateAtFullCapacity } from '../domain/roomLayout'
...
export function UnitsPage(){
  ...
  return <>
    <PageHeader eyebrow="Property records" title="Units & room layouts" copy="Give every unit its own room mix, capacity, and standard monthly price per occupant." action={<button className="primary" onClick={()=>setSelectedUnitId(units[0]?.id||null)}><BedDouble size={16}/> Manage rooms</button>}/>
...
function RoomManager({unit,onClose,onAdd,onUpdate}:{unit:Unit;onClose:()=>void;onAdd:(room:RoomDetails)=>void;onUpdate:(roomId:string,room:RoomDetails)=>void}){
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><div className="modal room-manager" role="dialog" aria-modal="true" aria-labelledby="room-title"><div className="dialog-head"><div><p className="eyebrow">Unit configuration</p><h2 id="room-title">Rooms in {unit.code}</h2><p>{unit.property} · {unit.rooms.length} room(s) · {roomSummary([unit]).capacity} total spaces</p></div><button type="button" className="icon-button close-dialog" onClick={onClose} aria-label="Close">×</button></div>
    {!adding&&!editing&&<><div className="managed-room-list">{unit.rooms.map(room=><div className="managed-room" key={room.id}><div><strong>{room.name}</strong><span>{room.type} room · {room.capacity} {room.capacity===1?'person':'people'} maximum · {money(room.rate)} per occupant</span><small>{room.occupants.length?`${room.occupants.length} assigned: ${room.occupants.join(', ')}`:'Currently empty'}</small></div><button className="secondary compact" onClick={()=>setEditing(room)}><Pencil size={14}/> Edit</button></div>)}</div><div className="notice-box">Room capacities and standard prices are independent. Tenant contracts can use a different rate for short stays or special arrangements.</div><div className="dialog-actions"><button type="button" className="secondary" onClick={onClose}>Done</button><button type="button" className="primary" onClick={()=>setAdding(true)}><Plus size={15}/> Add another room</button></div></>}
